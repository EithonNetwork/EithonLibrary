...
public class CommandSyntax extends Syntax {	
	...
	public CommandExecutor parse(Queue<String> argumentQueue, HashMap<String, ParameterValue> parameterValues) throws CommandArgumentException {
		if (this._subCommands.size() > 0) {
			...
			return commandSyntax.parse(argumentQueue, parameterValues);
		}

		for (ParameterSyntax parameterSyntax : this._parameterSyntaxList) {
			parameterSyntax.parse(argumentQueue.poll(), parameterValues);
		}
